<?php require_once("../clases/ClassAsignaturas.php");
$Asignatura= new ClassAsignaturas();
/**
...
function getData()
{
	$Asignatura= new ClassAsignaturas();
    $result=array();
	if(is_array($Asignatura->mostrarAsignaturas(trim($_GET['search']))))
	{
		foreach($Asignatura->mostrarAsignaturas(trim($_GET['search'])) as $a)
		{
		$result[]=$a["NOMBRE"]."-".$a["CODIGO"];
