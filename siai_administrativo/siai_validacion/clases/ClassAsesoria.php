...
class Asesoria {
    ...
    public function getPendientesAprobacion($iLimit, $codCarrera) {
        $consulta = "SELECT DISTINCT(a.CARNET)FROM asesoria as a 
		 INNER JOIN siai_usuario AS b ON a.CARNET=b.CARNET 
		 INNER JOIN siai_control AS c ON b.usuario=c.usuario
		 INNER JOIN expedientealumno AS ex
       	  on ex.carnet=b.carnet
		 WHERE a.MARCAR='0' AND a.CUPO='0'
		  and ex.CODCARRERA in(" . $codCarrera . ") 
		 LIMIT $iLimit,15";
        //var_dump($consulta);
    ...
    public function transaccionAsesoria() {
        ...
        $consulta = "INSERT INTO asesoria (CARNET, CODIGO_ASI, MATRICULA, SECCION, CODIGO_USU, FECHA_INGR, MARCAR, ARANCEL, CUPO, CICLO) VALUES ('" . $this->carnet . "', '" . $this->codigoasi . "', '" . $this->matricula . "', '" . $this->seccion . "', NULL, '" . $this->fechaingr . "', b'0', '" . $this->arancel . "', '" . $this->cupo . "', '" . $this->ciclo . "');";
    ...
    public function updateAsesoria() {
        $consulta = "UPDATE asesoria SET SECCION='" . $this->seccion . "', MARCAR='" . $this->marcar . "', CUPO='" . $this->cupo . "' WHERE CARNET='" . $this->carnet . "' AND CODIGO_ASI='" . $this->codigoasi . "' AND MATRICULA='" . $this->matricula . "';";
        $this->conexionAsesoria->conectarse();
        return $this->conexionAsesoria->ejecutarQuery($consulta);
    }

    public function borrarRegistro() {
        $consulta = "DELETE FROM asesoria WHERE CARNET='" . $this->carnet . "' AND CODIGO_ASI='" . $this->codigoasi . "' AND MATRICULA='" . $this->matricula . "' LIMIT 1;";
        $this->conexionAsesoria->conectarse();
        return $this->conexionAsesoria->ejecutarQuery($consulta);
    }
